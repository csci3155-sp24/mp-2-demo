...
class CanvasTests extends AnyFunSuite {
    ...
    test("Test Scale function") {
        /* We are not writing test cases for the scale 
          ...
          on your own? At the very least something like `assert(true)` 
          would ensure you pass this test case. Do keep in mind that 
          we are not grading you for this test case.*/
        assert(false)
    }
}
